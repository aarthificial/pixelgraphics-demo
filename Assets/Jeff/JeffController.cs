...
namespace Jeff
{
    ...
    public class JeffController : MonoBehaviour
    {
        ...
        private void EnterHitState(Collision2D collision)
        {
            ...
            var direction = _rigidbody2D.centerOfMass - relativePosition;
            if (Mathf.Abs(direction.x) < 0.2f)
                direction.x = 0.2f * Mathf.Sign(direction.x);
            
            hitStopwatch.Split();
            _rigidbody2D.AddForce(
                direction.normalized * bounceBackStrength - _rigidbody2D.velocity,
                ForceMode2D.Impulse
            );
        }
